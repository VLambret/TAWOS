...
def save_as_graph(project, title: str, x_label, y_label, all_data_to_plot: Dict[str, Dict[int, float]]):
    ...
    for label, data in all_data_to_plot.items():
        ...
        estimates_dates = data.keys()
        estimates_values = data.values()
...
def save_as_graph_old(project, title: str, x_label, y_label, all_data_to_plot: dict[str, CumulativeTimeSeries]):
    figure = Figure(figsize=(8, 6))
